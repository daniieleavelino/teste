document.addEventListener("DOMContentLoaded", function () {
  ...
  if (techCards.length > 0) {
    let idleIndex = 0;
    setInterval(() => {
      const currentCard = techCards[idleIndex];
      if (currentCard.getAttribute("aria-expanded") !== "true") {
        currentCard.classList.add("active");
        setTimeout(() => {
          currentCard.classList.remove("active");
        }, 900);
      }
      idleIndex = (idleIndex + 1) % techCards.length;
    }, 1600);
  }
